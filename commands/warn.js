    ...
    async run (client, message, args) {
		...
		if(!message.guild.me.hasPermission("MANAGE_CHANNELS")) return message.channel.send('<a:alert:995652726543355975> **I do not have the correct permissions | Permission : MANAGE_CHANNELS**')
		  ...
		  if (message.mentions.users.first().bot) {
			return message.channel.send("<a:alert:995652726543355975> **You can not warn bots**");
		  }
	  
		  if (message.author.id === user.id) {
			return message.channel.send("<a:alert:995652726543355975> **You can not warn yourself -_-**");
		  }
	  
		  if (user.id === message.guild.owner.id) {
			return message.channel.send(
			  "<a:alert:995652726543355975> **Bruh, you can not warn server owner -_-**"
			);
		  }
		  ...
		  if (!reason) {
			return message.channel.send(
			  "<a:alert:995652726543355975> **Please provide reason to warn**"
			);
		  }
		  ...
		  if (warnings === null) {
			...
			.setColor(embedColor)
			.setDescription(`<a:verify:995645296736481320> **User Successfully Warned**`)
