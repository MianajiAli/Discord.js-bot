...
module.exports = {
    ...
    async run (client, message, args) {
		if (!message.member.hasPermission("MUTE_MEMBERS")) {
			return message.channel.send("<a:alert:995652726543355975> **You can not use this command | Permission: MUTE_MEMBERS**");
		  }
		  ...
		  if (!user) {
			return message.channel.send("<a:alert:995652726543355975> **Please mention the member to who you want to unmute**");
		  }
	  
		  let muterole = message.guild.roles.cache.find(x => x.name === "Muted");
	  
		  if (user.roles.cache.has(muterole)) {
			return message.channel.send("<a:alert:995652726543355975> **This user dont have a Muted role**");
		  }
