    ...
    async run (client, message, args) {
		...
		try {
			...
			if (lott !== null && cooldown2 - (Date.now() - lott) > 0) {
				...
				return message.channel.send(`<a:alert:995652726543355975> **You can enter in lottery every month**`);
			} else {
				db.add(`money_${message.author.id}`, amount)
	        	db.set(`daily_${message.author.id}`, Date.now())
				return message.channel.send(`<a:verify:995645296736481320> **WOW ${message.author.tag}, You won ${amount}**`);
			}
