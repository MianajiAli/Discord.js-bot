...
module.exports = {
    ...
    async run (client, message, args) {
		...
		if (!message.guild.me.hasPermission("MANAGE_ROLES")) {
			return message.channel.send("<a:alert:995652726543355975> **I do not have the correct permissions | Permission : MANAGE_ROLES**");
		  }

		let channel = message.mentions.channels.first();
		if(!channel) return message.channel.send(`<a:alert:995652726543355975> **${prefix}rr <#channeL> <MESSAGEID> <ROLE> <EMOJI>**`)
			if(!args[1]) return message.channel.send(`<a:alert:995652726543355975> **${prefix}rr ${channel} <MESSAGEID> <ROLE> <EMOJI>**`)        
		
		let messageid = client.channels.cache.get(`${channel.id}`).messages.fetch(`${args[1]}`)
		 if(!messageid) return message.channel.send(`<a:alert:995652726543355975> **That's not an vaild message ID** `)
		
		if(isNaN(args[1])) return message.channel.send(`<a:alert:995652726543355975> **Message ID Must be a number**`)
		let role = message.mentions.roles.first();
		if(!role) return message.channel.send(`<a:alert:995652726543355975> **${prefix}rr ${channel} ${args[1]} <@role> <Emoji>**`)
		let check = message.guild.roles.cache.find(r => r.name === `${role.name}`)
		if(!check) return message.channel.send(`<a:alert:995652726543355975> **invaild role**`)
		if(!args[3]) return message.channel.send(`<a:alert:995652726543355975> **${prefix}rr ${channel} ${args[1]} ${role.name} <EMOJI> **`)
	 ...
	 .setTitle(`<a:verify:995645296736481320> Reaction Role`)
	 .setDescription(`**<a:verify:995645296736481320> Reaction Role Sucsses**
