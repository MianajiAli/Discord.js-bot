...
module.exports = {
    ...
    async run (client, message, args) {
		if (!message.member.hasPermission("ADMINISTRATOR")) {
		  ...
		  if (!user) {
			return message.channel.send("<a:no:997879343999111280> **Please mention the user for mute**");
		  }
		  if (user.id === message.author.id) {
			return message.channel.send("<a:no:997879343999111280> **I can't mute you because you are message author**");
		  }
		  let reason = args.slice(1).join("");
	  
		  if (!reason) {
			return message.channel.send("<a:no:997879343999111280> **Please give some reason for mute** ");
		  }
