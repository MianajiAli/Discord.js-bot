...
module.exports = {
    ...
    async run (client, message, args) {
		...
		if (!message.member.hasPermission("MANAGE_EMOJIS")) {
            return message.channel.send(`<a:alert:995652726543355975> **You can not use this command | Permission: MANAGE_EMOJIS**`)
        }
        if(!message.guild.me.hasPermission("MANAGE_EMOJIS")) return message.channel.send('<a:alert:995652726543355975> **I do not have the correct permissions**')
let emojis = await fetch("https://emoji.gg/api/").then(res => res.json());
     ...
     let noResult = new Discord.MessageEmbed()
        .setDescription(`<a:alert:995652726543355975> **No Results found for ${args.join(" ")}**`)
...
rmsg.reactions.resolve(reaction.emoji.name).users.remove(message.author.id).catch(err => {})
      ...
      try { 
        ...
        message.channel.stopTyping();
        message.channel.send(`<a:alert:995652726543355975> **Unable to add ${res.title}**`);
        rmsg.reactions.resolve(reaction.emoji.name).users.remove(message.author.id).catch(err => {})
        doing = false;
        break;
       }
       message.channel.send(`<a:verify:995645296736481320> **Successfully added ${created}**`);
      rmsg.reactions.resolve(reaction.emoji.name).users.remove(message.author.id).catch(err => {})
       doing = false;
       break;
 
     } else if (reaction.emoji.name == emojis[3]) {
       message.channel.send("<a:alert:995652726543355975> **Cancelled command**");
