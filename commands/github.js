...
module.exports = {
    ...
    async run (client, message, args) {
        try {
            ...
            if (!args[0]) return message.channel.send(`<a:no:997879343999111280> **Please enter an account name**`)
              
            fetch(`https://api.github.com/users/${args.join('-')}`)
              .then(res => res.json()).then(body => {
                if(body.message) return message.channel.send(`<a:no:997879343999111280> **User not found, Please enter a valid username**`);
              let { login, avatar_url, name, id, html_url, public_repos, followers, following, location, created_at, bio } = body;
                      ...
                      return message.channel.send(`<a:no:997879343999111280> **Something Went Wrong Try Again Later**`)
                  }
