		...
		if (!args[0]) {
		  ...
		  if (message.guild.channels.cache.has(b)) {
			...
			try {
				...
				if (channel.id === a) {
					return message.channel.send("**<:pepeno:997843592955580466> This Channel is Already Set As Rank Channel**")
				} else {
					client.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("<:peppoyes:997843596290052177> **Rank Channel Set**")
					db.set(`channel_${message.guild.id}`, channel.id)
	
					message.channel.send(`<:peppoyes:997843596290052177> **Rank Channel Has Been Set Successfully in \`${channel.name}\`**`)
				}
			} catch {
