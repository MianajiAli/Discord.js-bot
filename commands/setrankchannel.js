		...
		if (!args[0]) {
		  ...
		  if (message.guild.channels.cache.has(b)) {
			...
			try {
				...
				if (channel.id === a) {
					return message.channel.send("**<a:alert:995652726543355975> This Channel is Already Set As Rank Channel**")
				} else {
					client.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("<a:verify:995645296736481320> **Rank Channel Set**")
					db.set(`channel_${message.guild.id}`, channel.id)
	
					message.channel.send(`<a:verify:995645296736481320> **Rank Channel Has Been Set Successfully in \`${channel.name}\`**`)
				}
			} catch {
