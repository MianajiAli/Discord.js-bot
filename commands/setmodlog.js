		...
		if (!args[0]) {
		  ...
		  if (message.guild.channels.cache.has(b)) {
			...
			try {
				...
				if (channel.id === a) {
					return message.channel.send("**<a:no:997879343999111280> This Channel is Already Set As Modlog Channel**")
				} else {
					client.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("<a:yes:997879349170684064> **Modlog Channel Set**")
					db.set(`modlog_${message.guild.id}`, channel.id)
	
					message.channel.send(`<a:yes:997879349170684064> **Modlog Channel Has Been Set Successfully in \`${channel.name}\`**`)
				}
			} catch {
