...
module.exports = {
    ...
    async run (client, message, args) {
		if (!args[0]) return message.channel.send("<a:alert:995652726543355975> **You need to mention the role**")
        let role = message.mentions.roles.first() || message.guild.roles.cache.get(args[0]) || message.guild.roles.cache.find(r => r.name.toLowerCase() === args.join(' ').toLocaleLowerCase());
        if (!role) return message.channel.send("<a:alert:995652726543355975> **Please enter a valid role**");
