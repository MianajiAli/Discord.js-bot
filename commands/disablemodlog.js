...
module.exports = {
    ...
    async run (client, message, args) {
        ...
        try {
            ...
            if (!a) {
                return message.channel.send('<:pepeno:997843592955580466> **There Is No Modlog Channel Set To Disable**')
            } else {
                let channel = message.guild.channels.cache.get(a)
                client.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("<:pepeno:997843592955580466> **ModLog Channel Disabled**")
                db.delete(`modlog_${message.guild.id}`)

                message.channel.send(`<:peppoyes:997843596290052177> **Modlog Channel Has Been Successfully Disabled in \`${channel.name}\`**`)
            }
            return;
        } catch {
