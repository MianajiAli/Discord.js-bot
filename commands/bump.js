    ...
    async run (client, message, args) {
		...
		try {
			...
			if (point !== null && cooldown2 - (Date.now() - point) > 0) {
				return message.channel.send(`<a:alert:995652726543355975> **Please wait another 2 hours until the server can be bumped**`);
			} else {
				...
				.setDescription(`<a:verify:995645296736481320> **Server bumped** \n**Type \`${prefix}svpoints\` to see the server points**`)
