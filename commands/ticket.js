...
module.exports = {
    ...
    async run (client, message, args) {
		...
		if(message.guild.channels.cache.find(channel => channel.name === `ticket-${message.author.id}`)) {
			return message.reply('<a:alert:995652726543355975> **You already have a ticket, please close your exsisting ticket first before opening a new one**');
		}
		...
		message.guild.channels.create(`ticket-${message.author.id}`, {
			...
			.setColor(color.main)
			message.reply(`<a:verify:995645296736481320> **You have successfully created a ticket, Please click on ${channel} to view your ticket**`).then(m => m.delete({ timeout: 14000 }).catch(e => {}));
